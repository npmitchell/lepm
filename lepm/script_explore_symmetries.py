...
for series in ['gammapt', 'kpt']:
    ...
    if not done:
        ...
        for (pp, ii) in zip(proj, range(len(proj))):
            ...
            plt.savefig(outdir + 'proj_p2minusp/' + series + '/p2minusp_{0:03d}_'.format(ii) + '.png')
            plt.clf()
    ...
    imgname = outdir + 'sumproj/' + series + '/sumproj_bands0through'
    movname = outdir + 'sumproj_' + series
    lemov.make_movie(imgname, movname, indexsz='03', framerate=fr)
