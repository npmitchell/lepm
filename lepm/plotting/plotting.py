...
def DOS_plot(eigval, DOS_ax, sim_type, pin=-5000, alpha=1.0, colorV=None, facecolor='#80D080', nbins=75, fontsize=12,
    ...
    hist_vals[np.isnan(hist_vals)] = 0.
    print 'leplt: hist_vals = ', hist_vals
    # sys.exit()
...
def colored_DOS_plot(eigval, DOS_ax, sim_type, alpha=1.0, colorV=None, colormap='viridis', norm=None,
    ...
    if colorV is not None:
        ...
        if norm is None:
            ...
            if vmax is None:
                if climbars:
                    vmax = np.max(colors)
                    print 'leplt: vmax --> climbars:', vmax
                else:
                    vmax = np.max(colorV)
                    print 'leplt: vmax --> climbars:', vmin
            norm = matplotlib.colors.Normalize(vmin=vmin, vmax=vmax)
            sm = matplotlib.cm.ScalarMappable(cmap=colormap, norm=norm)
        else:
            norm = mpl.colors.LogNorm(vmin=0.0, vmax=1.0)
            raise  RuntimeError('Have not coded to allow for manual normalization using norm kwarg')
    ...
    if ylabel_right:
        print 'plotting.plotting: setting ylabel position to be right'
...
def shaded_DOS_plot(eigval, DOS_ax, sim_type, alpha=None, facecolor='#80D080', nbins=75,
    ...
    if alpha is not None:
        if len(alpha) == nbins:
            ...
            raise RuntimeError(
                "Supplied alpha (array of opacities) must have same length as eigval or as nbins (#bins).")
...
def plot_eigvect_excitation(xy, fig, dos_ax, eig_ax, eigval, eigvect, en, marker_num=0,
    ...
    # Pick a series of times to draw out the ellipsoid
    time_arr = np.arange(81) * 2 * np.pi / (np.abs(ev) * 80)
    if theta is None:
        exp1 = np.exp(ev * time_arr)
    else:
        exp1 = np.exp(ev * time_arr) * np.exp(1j * theta)
    ...
    for i in range(NP):
        ...
        if color == 'phase':
            ...
            angles_arr[i] = anglez
            colors[i] = anglez
        elif color == 'displacement':
            colors[i] = np.sqrt(np.mean(x_vals ** 2 + y_vals ** 2))

    pp = PatchCollection(patch, cmap=cmap, lw=lw, alpha=alpha, zorder=zorder)
    ...
    # this is the part that puts a dot a t=0 point
    if mark_t0:
        scat_fg = eig_ax.scatter(x0s, y0s, s=s(t0_ptsize), c='k')
    else:
        scat_fg = []

    # this is the part that puts a line from the center of each particle to the position at t=0
    if black_t0lines:
        lines_12 = [zip(x, y) for x, y in lines_1]
        lines_12_st = LineCollection(lines_12, linewidth=0.8)
        lines_12_st.set_color('k')
...
def timestep_plot(current_pos, xy, Ni, Nk, BM, ax=None, factor=1, amp=1, title='', color_particles='k',
                  color_excitation='angles', excitation_clims=None,
                  ...
                  ticks_off=True, alpha=0.6, excite_color=None, cmap_excite='isolum_rainbow',
                  show_bonds=True, boundary=None, rasterized=False):
    ...
    if 'none' not in bgcolor and bgcolor is not None:
    ...
    if excite_color is not None:
        p = PatchCollection(patch, color=excite_color, alpha=alpha, edgecolor=circ_edgecolor,
                            linewidth=circ_linewidth, rasterized=rasterized)
    else:
        p = PatchCollection(patch, cmap=cmap_excite, alpha=alpha, edgecolor=circ_edgecolor,
                            linewidth=circ_linewidth, rasterized=rasterized)
        if color_excitation == 'angles':
            p.set_array(np.array(angles))
            p.set_clim([0, 2. * np.pi])
        elif color_excitation == 'magnitude':
            # print 'np.max(mag) = ', np.max(mag)
            # sys.exit()
            p.set_array(np.array(mag))
            if excitation_clims is not None:
                p.set_clim([excitation_clims[0], excitation_clims[1]])
                # mag[mag > excitation_clims[1]] = excitation_clims[1]
                # mag[mag < excitation_clims[0]] = excitation_clims[0]
    ...
    if color_particles == 'k':
        ...
        scat_fg0 = ax.scatter(scat_x[bulkinds], scat_y[bulkinds], s=blacksz,
                              facecolor='k', edgecolors='w', lw=lwsz,
                              alpha=1.0, zorder=70, rasterized=rasterized)
        scat_fg1 = ax.scatter(scat_x[bulkinds], scat_y[bulkinds], s=whitesz, c='w',
                              alpha=1.0, zorder=80, rasterized=rasterized)
        scat_fg2 = ax.scatter(scat_x[boundary], scat_y[boundary], s=whitesz, c='k',
                              alpha=1.0, zorder=60, rasterized=rasterized)
        scat_fg = [scat_fg0, scat_fg1, scat_fg2]

    elif isinstance(color_particles, np.ndarray) or isinstance(color_particles, float):
        # print 'coloring based on array'
        scat_fg = ax.scatter(scat_x, scat_y, s=s(5 * ptsize), c=color_particles.reshape(1, -1),
                             lw=0.6, edgecolor='k', cmap=scat_cmap,
                             vmin=0., vmax=1.0, zorder=999999, rasterized=rasterized)
    else:
        scat_fg = []
...
def initialize_2panel_1cbar_centy(Wfig=360, Hfig=270, fontsize=12, wsfrac=0.4, wssfrac=0.3, x0frac=0.1, hspace=10,
