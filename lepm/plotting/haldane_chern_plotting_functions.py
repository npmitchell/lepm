...
def plot_spectrum_with_dos(hlat, ksys_fracM, omegacM, nuM, ngrid, Wfig=90, Hfig=None, x0frac=0.15, y0frac=0.1,
    ...
    hlat.add_ipr_to_ax(ax[0], ipr=None, alpha=1.0, inverse_PR=False,
...
def plot_projector_locality(haldane_lattice, dists, magproj, evxymag, outdir=None, network_str='none', show=False,
                            alpha=None):
    """
    ...
    if outdir is None:
        outdir = le.prepdir(haldane_lattice.lp['meshfn'].replace('networks', 'projectors'))
    le.ensure_dir(outdir)
    plt.savefig(outdir + haldane_lattice.lp['LatticeTop'] + '_projector_log.png', dpi=300)

    ax[0].set_xlim(-0.5, 5)
    ax[0].set_ylim(-3.5, 0.5)
    plt.savefig(outdir + haldane_lattice.lp['LatticeTop'] + '_projector_zoom_log.png', dpi=300)
    if show:
        plt.show()
    plt.clf()
    ...
    if outdir is None:
        outdir = le.prepdir(haldane_lattice.lp['meshfn'].replace('networks', 'projectors'))
    le.ensure_dir(outdir)
    plt.savefig(outdir + haldane_lattice.lp['LatticeTop'] + '_projector_xy2_log.png', dpi=300)

    ax[0].set_xlim(-0.5, 5)
    ax[0].set_ylim(-3.5, 0.5)
    plt.savefig(outdir + haldane_lattice.lp['LatticeTop'] + '_projector_xy2_zoom_log.png', dpi=300)
    if show:
        plt.show()
    plt.clf()
...
def plot_projector_locality_singlept(haldane_lattice, proj_ind, dists, magproj, ax=None, save=True,
    ...
    if save:
        if outdir is None:
            outdir = le.prepdir(haldane_lattice.lp['meshfn'].replace('networks', 'projectors'))
        le.ensure_dir(outdir)
        plt.savefig(outdir + haldane_lattice.lp['LatticeTop'] + '_projector_xy2_log_{0:06d}'.format(proj_ind) +
                    '.png', dpi=300)

        ax.set_xlim(-0.5, 5)
        ax.set_ylim(-3, 0)
        plt.savefig(outdir + haldane_lattice.lp['LatticeTop'] + '_projector_xy2_zoom_log_{0:06d}'.format(proj_ind) +
                    '.png', dpi=300)

        # save dists and magproj as pkl
        outfn = outdir + haldane_lattice.lp['LatticeTop'] + "_dists_singlept_{0:06d}".format(proj_ind) + ".pkl"
        with open(outfn, "wb") as fn:
            pickle.dump(dists[proj_ind], fn)
        with open(outdir + haldane_lattice.lp['LatticeTop'] + "_magproj_{0:06d}".format(proj_ind) + ".pkl", "wb") as fn:
            pickle.dump(magproj[proj_ind], fn)
    if show:
        plt.show()


def plot_projector_singlept_network(hlat, evxyproj, fig=None, ax=None, cax=None,
    ...
    **kwargs : keyword arguments for leplt.initialize_1panel_cbar_cent()
    ...
    hlat.lattice.plot_BW_lat(fig=None, ax=ax, save=False, close=False, axis_off=axis_off, title='')
    if cmap not in plt.colormaps():
        lecmaps.register_colormaps()
    colormap = plt.get_cmap(cmap)
    colors = colormap(magproj/np.max(magproj))
    # print 'colors = ', colors
    ax.scatter(hlat.lattice.xy[:, 0], hlat.lattice.xy[:, 1], s=sz, c=colors, edgecolors='none', zorder=200)
...
if __name__ == '__main__':
    ...
    from lepm import haldane_collection
