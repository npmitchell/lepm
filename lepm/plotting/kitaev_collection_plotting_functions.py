...
def movie_cherns_varyloc(ccoll, title='Chern number calculation for varied positions',
    ...
    # plot it
    for glat_name in ccoll.cherns:
        glat = ccoll.cherns[glat_name][0].gyro_lattice
        ...
        ksize = ccoll.cherns[glat_name][0].chern_finsize[:, 2]
        cgll = ccoll.cherns[glat_name][0].gyro_lattice.lattice
        ...
        glat_cmesh = kfns.get_cmeshfn(ccoll.cherns[glat_name][0].gyro_lattice.lp, rootdir=rootdir)
        glat = ccoll.cherns[glat_name][0].gyro_lattice
        netvis.movie_plot_2D(glat.lattice.xy, glat.lattice.BL, 0 * glat.lattice.BL[:, 0],
        ...
        if title is not None:
            ax[0].annotate(title, xy=(0.5, .97), xycoords='figure fraction',
        ...
        # Check if all images have been written already (default is to not overwrite pngs)
        specstr = '_Nks' + '{0:03d}'.format(len(ksize)) + '_step' + sf.float2pstr(step) \
                  + '_maxbsz' + sf.float2pstr(max_boxsize)
        outdir = glat_cmesh + '_' + glat.lp['LatticeTop'] + '_varyloc_stills' + specstr + '/'
        if len(glob.glob(outdir + filename + specstr + '_*' + exten)) > len(ccoll.cherns[glat_name]) - 1:
            done = True
        else:
            done = False
        ...
        while done is False:
            rectps = []
            colorL = []
            for chernii in ccoll.cherns[glat_name]:
                ...
                if max_boxsize is not None:
                    ...
                    if max_boxfrac is not None:
                        cgll = chernii.gyro_lattice.lattice
                        maxsz = max(np.max(cgll.xy[:, 0]) - np.min(cgll.xy[:, 0]),
                                    np.max(cgll.xy[:, 1]) - np.min(cgll.xy[:, 1]))
                        max_boxsize = max_boxfrac * maxsz
                        ksize = ksize[ksize < max_boxsize]
        ...
        movname = glat_cmesh + filename + specstr + '_mov'
...
def movie_cherns_varyloc_moving_kregion(ccoll, title='Chern number calculation for varied positions',
                                        filename='chern_varyloc', rootdir=None, exten='.png', boxsize=5.0,
                                        xlabel=None, ylabel=None, step=0.5, fracsteps=False, framerate=3,
                                        randomloc=False,
                                        firstlocs='0.57/0.42/n0.1/n0.05/0.08/0.7/n0.73/n0.28/0.5/n.8/n0.6/0.4',
                                        modrandframes=100):
    """Plot the chern as a function of space for each gyro_lattice examined, making a movie with the kitaev region on
    the lattice on the left, and the chern-colored lattice on the right
    ...
    framerate : int
        The framerate at which to save the movie
    randomloc : bool
        Whether to probe kitaev region locations in random fashion
    firstlocs : str
        x0/y0/x1/y1/...xn/yn as a string, for the first n positions of the kitaev regions as a fraction of the max
        possible system radius/extent (where extent = np.max(np.abs(xy.ravel())))
    """
    rad = 1.0
    divgmap = cmaps.diverging_cmap(250, 10, l=30)
    ...
    for glat_name in ccoll.cherns:
        ...
        y0frac = 0.06
        hspace = 0.02 * w_mm
        fig, ax, cax = leplt.initialize_nxmpanel_cbar_fig(1, 2, Wfig=w_mm, Hfig=h_mm, wsfrac=wsfrac,
                                                           x0frac=x0frac, y0frac=y0frac, wcbarfrac=0.4, hcbarfrac=0.03,
                                                           orientation='horizontal', x0cbarfrac=None, hspace=hspace,
                                                           y0cbarfrac=None, cbar_placement='above_right')
        ...
        for current_ax in ax:
            netvis.movie_plot_2D(glat.lattice.xy, glat.lattice.BL, 0 * glat.lattice.BL[:, 0],
                                 None, None, ax=current_ax, fig=fig, axcb=None,
                                 xlimv='auto', ylimv='auto', climv=0.1, colorz=False, ptcolor=None, figsize='auto',
                                 colormap='BlueBlackRed', bgcolor='#ffffff', axis_off=True, axis_equal=True,
                                 lw=0.2)
        ...
        # fake up the array of the scalar mappable.
        sm._A = []
        cbar = plt.colorbar(sm, cax=cax, orientation='horizontal', ticks=[-1, 0, 1])
        cbar.set_label(r'Chern number, $\nu$', labelpad=-30, fontsize=8)
        ax[0].axis('off')
        ax[1].axis('off')
        ...
        # Get output naming path and strings
        # First get boxsize from the first ksize array
        ksize = ccoll.cherns[glat_name][0].chern_finsize[:, 2]
        szind = np.argmin(np.abs(ksize - boxsize))
        ksize = ksize[szind]
        # Then define more strings and outdir
        glat_cmesh = kfns.get_cmeshfn(ccoll.cherns[glat_name][0].gyro_lattice.lp, rootdir=rootdir)
        specstr = '_Nks' + '{0:03d}'.format(len(ccoll.cherns[glat_name][0].chern_finsize[:, 2])) + \
                  '_step' + sf.float2pstr(step) + '_bsz' + sf.float2pstr(ksize)
        outdir = glat_cmesh + '_' + glat.lp['LatticeTop'] + '_varyloc_'
        if randomloc:
            outdir += 'randommovingkreg_stills' + specstr + '/'
        else:
            outdir += 'movingkreg_stills' + specstr + '/'
        ...
        print 'len(glob...) => ' + outdir + filename + specstr + '_*' + exten
        print '---> ', len(glob.glob(outdir + filename + specstr + '_*' + exten))

        # Resort in random order if randomloc is True
        if randomloc:
            # Get indices of cherns with kregs that are closest to target locations
            # First get the realspace locations of where to look for the first n kitaev regions
            firstlocs = sf.string_sequence_to_numpy_array(firstlocs, dtype=float)
            firstlocs = firstlocs.reshape(int(len(firstlocs) * 0.5), 2)
            lat = ccoll.cherns[glat_name][0].gyro_lattice.lattice
            extent = np.max(np.abs(lat.xy.ravel()))
            firstlocs *= extent
            loc = np.zeros((len(ccoll.cherns[glat_name]), 2), dtype=float)
            ii = 0
            for chernii in ccoll.cherns[glat_name]:
                xx = float(chernii.cp['poly_offset'].split('/')[0])
                yy = float(chernii.cp['poly_offset'].split('/')[1])
                loc[ii] = np.array([xx, yy])
                ii += 1

            # get closest index
            locinds = []
            for floc in firstlocs:
                dists = floc - loc
                locinds.append(np.argmin(dists[:, 0]**2 + dists[:, 1]**2))

            sortind = np.setdiff1d(np.arange(len(ccoll.cherns[glat_name])), locinds).tolist()
            random.shuffle(sortind)
            sorted_cherns1 = [ccoll.cherns[glat_name][ind] for ind in locinds]
            sorted_cherns2 = [ccoll.cherns[glat_name][ind] for ind in sortind]
            sorted_cherns = sorted_cherns1 + sorted_cherns2
        else:
            sorted_cherns = ccoll.cherns[glat_name]

        # ind is for determining which kreg locations to plot, while randind is for output index if randomloc==True
        ind = 0
        randind = 0
        while done is False:
            # if randomloc, make many frames in the beginning
            if randomloc:
                # determine how many copies to write to file so that first few frames are shown for longer time
                if ind == 0:
                    nframes = 1
                elif ind == 1:
                    # approx 1.5 seconds
                    nframes = max(int(1.5 * framerate), 1)
                elif ind == 2:
                    # approx 0.7 seconds
                    nframes = max(int(0.75 * framerate), 1)
                elif ind == 3:
                    nframes = max(int(0.40 * framerate), 1)
                elif ind == 4:
                    nframes = max(int(0.20 * framerate), 1)
                elif ind == 5:
                    nframes = max(int(0.10 * framerate), 1)
                elif ind == 6:
                    nframes = max(int(0.05 * framerate), 1)
                elif ind == 7:
                    nframes = max(int(0.02 * framerate), 1)
                elif ind == len(ccoll.cherns[glat_name]) or np.mod(ind, modrandframes) == 0:
                    nframes = 1
                else:
                    nframes = 0
            else:
            ...
            if nframes > 0:
                ...
                # If this is the first frame, skip the chern patches
                if ind > 0:
                    for chernii in sorted_cherns[0:ind]:
                        # Grab kitaev region that matches boxsize
                        ksize = chernii.chern_finsize[:, 2]
                        szind = np.argmin(np.abs(ksize - boxsize))
                        ksize = ksize[szind]
                ...
                # Draw the kitaev cartoon in left axis with size ksize[ind]
                polygon1, polygon2, polygon3 = kfns.get_kitaev_polygons(ccoll.cp['shape'], ccoll.cp['regalph'],
                                                                        ccoll.cp['regbeta'], ccoll.cp['reggamma'],
                                                                        ksize)

                # Unless this is the first frame, move the kitaev region to the location where the chern is computed
                if ind > 0:
                    polygon1 += np.array([xx, yy])
                    polygon2 += np.array([xx, yy])
                    polygon3 += np.array([xx, yy])

                patchlist = [patches.Polygon(polygon1, color='r'),
                             patches.Polygon(polygon2, color='g'),
                             patches.Polygon(polygon3, color='b')]
                polypatches = PatchCollection(patchlist, cmap=cm.jet, alpha=0.4, zorder=99, linewidths=0.4)
                colors = np.linspace(0, 1, 3)[::-1]
                polypatches.set_array(np.array(colors))
                ax[0].add_collection(polypatches)
                ax[0].set_xlim(ax[0].get_xlim())
                ax[0].set_ylim(ax[0].get_ylim())

            # Save the plot
            # make index string
            for tmpii in range(nframes):
                indstr = '_{0:06d}'.format(randind)
                fnout = outdir + filename + specstr + indstr + exten
                print 'saving figure: ' + fnout
                dio.ensure_dir(outdir)
                fig.savefig(fnout, dpi=actual_dpi * 2)
            ...
            if nframes > 0:
                # clear both the kitaev region patches and the collection of chern patches (from first to ind'th)
                if ind > 0:
                    p.remove()
                polypatches.remove()

            # Update index for indexing sorted_cherns
            ind += 1
            if ind == len(ccoll.cherns[glat_name]) + 1:
                done = True
        ...
        if randomloc:
            movname += '_randomloc'
        subprocess.call(['./ffmpeg', '-framerate', str(int(framerate)), '-i',
                         imgname + '_%06d' + exten, movname + '.mov',
                         '-vcodec', 'libx264', '-profile:v', 'main', '-crf', '12', '-threads', '0',
                         '-r', '30', '-pix_fmt', 'yuv420p'])
...
def plot_cherns_varyloc(ccoll, title='Chern number calculation for varied positions',
                        ...
                        dpi=600, fontsize=12, rasterized=False):
    """Plot the chern as a function of space for each gyro_lattice examined. If save==True, saves figure to
    ...
    for glat_name in ccoll.cherns:
        rectps = []
        colorL = []
        print 'all glats should have same pointer:'
        print 'ccoll[glat_name][0].gyro_lattice = ', ccoll.cherns[glat_name][0].gyro_lattice
        print 'ccoll[glat_name][1].gyro_lattice = ', ccoll.cherns[glat_name][1].gyro_lattice
        ...
        if maxchern:
            ...
            for chernii in ccoll.cherns[glat_name]:
                ...
                colorL.append(nu[ind])
                rectps.append(rect)
                max_boxsize = chernii.chern_finsize[ind, 2]
        else:
        ...
        p = PatchCollection(rectps, cmap=divgmap, alpha=1.0, edgecolors='none', rasterized=rasterized)
        p.set_array(np.array(np.array(colorL)))
        p.set_clim([-1., 1.])
        ...
        if ax is None:
            ...
            netvis.movie_plot_2D(glat.lattice.xy, glat.lattice.BL, 0*glat.lattice.BL[:, 0],
                                 ...
                                 colormap='BlueBlackRed', bgcolor='#ffffff', axis_off=True, axis_equal=True,
                                 lw=0.2, zorder=999, rasterized=rasterized)

            # Add title
            if title is not None and title != 'none':
                ax[0].annotate(title, xy=(0.5, 0.97), xycoords='figure fraction',
                               horizontalalignment='center', verticalalignment='top')
            if xlabel is not None:
                ax[0].set_xlabel(xlabel)
            if ylabel is not None:
            ...
            netvis.movie_plot_2D(glat.lattice.xy, glat.lattice.BL, 0*glat.lattice.BL[:, 0],
                             ...
                             xlimv='auto', ylimv='auto', climv=0.1, colorz=colorz, ptcolor=None, figsize='auto',
                             colormap='BlueBlackRed', bgcolor='#ffffff', axis_off=True, axis_equal=True,
                             lw=0.2, rasterized=rasterized)

            # Add title
            if title is not None and title != 'none':
                ax.annotate(title, xy=(0.5, 0.97), xycoords='figure fraction',
                            horizontalalignment='center', verticalalignment='top')
            if xlabel is not None:
...
def plot_chern_spectrum_on_axis(kcoll, ax, dos_ax, cbar_ipr_ax, cbar_nu_ax, fontsize=8, alpha=1.0,
    ...
    for glat_name in kcoll.cherns:
        ...
        dmyi += 1
    if dmyi > 1:
        raise RuntimeError('This function takes a KitaevCollection which is allowed only ONE gyro_lattice-- ' +
                           'ie, a chern spectrum for a single GyroLattice instance')

    print 'Opening glat_name = ', glat_name
    ngrid = len(kcoll.cherns[glat_name][0].chern_finsize)

    # Assume all ksize elements are the same size for now
    ksys_frac = kcoll.cherns[glat_name][0].chern_finsize[:, -2]*0.5
    ksys_fracM = np.array([ksys_frac for i in range(len(kcoll.cherns[glat_name]))])

    # Build omegacV
    omegacV = np.zeros(len(kcoll.cherns[glat_name]))
    nuM = np.zeros((len(kcoll.cherns[glat_name]), len(ksys_frac)))

    for ind in range(len(kcoll.cherns[glat_name])):
        print 'ind = ', ind
        cp_ii = kcoll.cherns[glat_name][ind].cp
        omegacV[ind] = cp_ii['omegac']
        nuM[ind, :] = kcoll.cherns[glat_name][ind].chern_finsize[:, -1]
...
def plot_nugrid_chern(chern, glat, xyvec, nugrids, repi, outdir=None, name_exten='', ax=None, fontsize=8):
    """Plot a pcolormesh of the spatially-resolved chern calcs on ax and plot the lattice on top of this.

    Parameters
    ----------
    chern : KitaevChern instance
...
def plot_gradnusum_chern(chern, glat, xyvec, nugsum, repi, outdir=None, name_exten='', ax=None, fontsize=8):
...
def plot_various_gnu_psi2(glat, gnu_psi2, eigval, top, repi, repi2, sz=1, outdir=None):
...
def plot_kcoll_gnusum_chern(ev, gnupsi2, cmap='copper_r', ax=None, cbar_ax=None, make_cbar=True,
    ...
    ev : N x 1 float array
        sorted eigenvalues of all gyro_lattices in the chern collection
