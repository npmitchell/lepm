...
def calc_magevecs(eigvect):
    """Compute the magnitude of all eigenvectors, by norming their x and y components in quad
    NOTE: This is a similar function to that in gyro_lattice_functions.py, but uses all 4 dof at each site

    Parameters
    ----------
    eigvect : 4*N x 4*N complex array
        ...
        x0, y0, x1, y1, ... xNP, yNP, w0, z0, w1, z1, ... wNP, zNP.
    ...
    # Contract x and y component of each evect so that len(halfevec[ii]) = NP
    numpts = len(eigvect) * 0.25
    magevecs = np.zeros((len(eigvect), numpts), dtype=float)
    jj = 0
    for row in eigvect:
        magevecs[jj] = np.array([np.sqrt(np.abs(row[2 * ii]) ** 2 + np.abs(row[2 * ii + 1]) ** 2 +
                                         np.abs(row[numpts + 2 * ii]) ** 2 + np.abs(row[numpts + 2 * ii + 1]) ** 2)
                                 for ii in np.arange(0, int(numpts))])
        jj += 1
...
def fit_eigvect_to_exponential_1dperiodic(xy, eigval, eigvect, LL, locutoffd=None, hicutoffd=None, check=False):
    ...
    fits : NP x 7 float array (ie, int(len(eigval)*0.5) x 7 float array)
        fit details: x_center, y_center, A, K, uncertainty_A, covariance_AK, uncertainty_K
    """
    # Contract x and y component of each evect so that len(eigval[ii]) = NP
    magevec = calc_magevecs(eigvect)
...
def fit_eigvect_to_exponential_edge(xy, boundary, eigvect, eigval=None, locutoffd=None, hicutoffd=None, check=False,
    ...
    fits = np.zeros((len(eigval), 5), dtype=float)
    for ii in range(len(eigval)):
        if ii % 100 == 1:
