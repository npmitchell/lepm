...
def calc_haldane_chern(haldane_lattice, cp, pp=None, check=False, contributions=False, verbose=False, vis_exten='.png',
                       contrib_exten='.pdf', attribute_evs=False, title=None):
    """Compute the chern number for a haldane_lattice
    ...
    # If we want to look at individual contributions from each site, find h first
    if contributions and NP < 500:
        ...
        print 'skipping 2-current h_ijk to do sum term by term...'
        method = 'projector'
    ...
    for kk in range(len(ksize_frac_arr)):
        ...
        if 'delta' in haldane_lattice.lattice.lp:
            # Note: by convention, lp['delta'] is in radians, whereas lp['delta_lattice'] is in radians/pi
            delta = haldane_lattice.lattice.lp['delta']/np.pi
        else:
            if haldane_lattice.lattice.lp['delta_lattice'] in ['0.667', '0p667']:
                delta = 2./3.
            else:
                if isinstance(haldane_lattice.lattice.lp['delta_lattice'], str):
                    delta = float(haldane_lattice.lattice.lp['delta_lattice'].replace('p', '.'))
                else:
                    delta = float(haldane_lattice.lattice.lp['delta_lattice'])
        ...
        if cp['basis'] == 'XY':
            reg1 = np.sort(reg1_xy.ravel())
            reg2 = np.sort(reg2_xy.ravel())
            reg3 = np.sort(reg3_xy.ravel())
        elif cp['basis'] == 'psi':
            if verbose:
                print 'stacking regions with right-moving selves...'
            reg1 = np.sort(reg1.ravel())
            reg2 = np.sort(reg2.ravel())
            reg3 = np.sort(reg3.ravel())
        ...
        if save_ims and (kk % modsave == 0 or kk == (len(ksize_frac_arr)-1)):
            ...
            # plot_chern_realspace(haldane_lattice, reg1_xy, reg2_xy, reg3_xy, polygon1, polygon2, polygon3,
            ...
            plot_haldane_chern_realspace_2panel(haldane_lattice, ksys_fracV, nuV, kk, reg1_xy, reg2_xy, reg3_xy,
            ...
            if contributions:
                ...
                plot_chern_contributions(haldane_lattice, reg1, reg2, reg3, cb1, cb2, cb3, polygon1, polygon2, polygon3,
...
def sum_kitaev_with_contributions_projector(reg1, reg2, reg3, r1old, r2old, r3old, pp, nu, verbose=False):
    """
    Keep track of contributions from individual sites as sum 2-current of the projector
...
def plot_haldane_chern_realspace_2panel(haldane_lattice, ksys_fracV, nuV, kk, reg1_xy, reg2_xy, reg3_xy,
    ...
    # Note that plot_chern_realspace accepts a simple xy as the haldane_lattice argument
    plot_chern_realspace(haldane_lattice, reg1_xy, reg2_xy, reg3_xy, polygon1, polygon2, polygon3,
    ...
    # Note difference from regular chern here: do not multiply ksys_fracV by 0.5
    ax[1].scatter(ksys_fracV[0:kk+1], nuV[0:kk+1], c=nuV[0:kk+1], edgecolor="none",
                  cmap=cmap, vmin=-1.0, vmax=1.0)
    ax[1].set_xlim(0, 1.0)
    ax[1].set_ylim(-1., 1.0)
    ax[1].set_xlabel(r'Fraction of sites in sum')
...
def plot_chern_contributions(haldane_lattice, reg1, reg2, reg3, nu_sum1, nu_sum2, nu_sum3, polygon1, polygon2, polygon3,
    ...
    reg1 : #sites in reg1 x 1 array
        indices of eigvector/eigvals for sites in reg3 (2*index for x, 2*index + 1 for y d.o.f.)
    reg2 : #sites in reg1 x 1 array
        indices of eigvector/eigvals for sites in reg3 (2*index for x, 2*index + 1 for y d.o.f.)
    reg3 : #sites in reg1 x 1 array
        indices of eigvector/eigvals for sites in reg3 (2*index for x, 2*index + 1 for y d.o.f.)
    nu_sum1 :
        partial contributions from each site in reg1
    nu_sum2 :
        partial contributions from each site in reg2
    nu_sum3 :
        partial contributions from each site in reg3
...
def get_cpmeshfn(cp, lp):
    ...
    cpmeshfn += '_polyoff' + sf.prepstr(cp['poly_offset'])
    cpmeshfn += '_' + cp['basis'] + '/'
    return cpmeshfn
