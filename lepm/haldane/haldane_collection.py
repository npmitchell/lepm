...
import lepm.plotting.haldane_collection_plotting_functions as hcpfns
...
class HaldaneCollection:
    """Create a collection of Haldane lattices. Lattices can exist in memory, on hard disk, or both.
    
    Attributes
    ----------
    self.haldane_lattices : list of instances of haldane_lattice_class.HaldaneLattice() corresponding to each network
    ...
    def add_haldane_lattice(self, glat):
        """Add haldane_lattice instance and its meshfn to current collection"""
        self.haldane_lattices.append(glat)
    ...
    def get_meshfns(self):
        try:
            ...
            meshfns = []
            for glat in lattice_collection.haldane_lattices:
                try:
    ...
    def load_haldane_lattices(self, load_DOS=True):
        # Use ensure_all_haldane_lattices() instead
    ...
    def load_eigvals(self):
        ...
        for ii in range(len(self.meshfns)):
            self.ensure_all_haldane_lattices()
            self.haldane_lattices[ii].load_eigval(attribute=True)

    def load_eigvects(self):
        """Ensure that for every haldane_lattice there is an eigvect array loaded"""
        for ii in range(len(self.meshfns)):
            self.ensure_all_haldane_lattices()
            self.haldane_lattices[ii].load_eigvect(attribute=True)

    def calc_max_polygon_sides(self):
        maxpno = 0
        for ii in range(len(self.haldane_lattices)):
            if self.haldane_lattices[ii].lattice.polygons is not None:
                polygons = self.haldane_lattices[ii].lattice.polygons
            else:
                polygons = self.haldane_lattices[ii].lattice.load_polygons()
    ...
    def ensure_all_haldane_lattices(self):
        ...
        for ii in range(len(self.meshfns)):
            ...
            try:
                ...
                lp['Omg'] = -1.0
                if append_glat:
                    self.haldane_lattices.append(haldane_lattice_class.HaldaneLattice(lat, lp))
                else:
                    self.haldane_lattices[ii] = haldane_lattice_class.HaldaneLattice(lat, lp)

                glat = haldane_lattice_class.HaldaneLattice(lat, self.haldane_lattices[ii].lp)
                self.haldane_lattices[ii] = glat

    def ensure_all_eigval(self):
        """Ensure eigenval is attribute for each haldane_lattice in haldane_collection"""
        self.ensure_all_haldane_lattices()

        # Make sure all eigvals are accounted for
        test = self.haldane_lattices[0].eigval is not None
        for ii in np.arange(1,len(self.haldane_lattices)):
            test = self.haldane_lattices[ii].eigval is not None and test
        print 'All haldane_lattices have eigval attribute, all set to overlay DOS...'
        if not test:
            print 'Some haldane_lattices do not already have eigval as an attribute, loading those now...'
            self.load_eigvals()

    def ensure_all_ipr_saved(self):
        self.ensure_all_haldane_lattices()
        for ii in range(len(self.haldane_lattices)):
            glat = self.haldane_lattices[ii]
            if not glob.glob(glat.lp['meshfn'] + '/ipr' + glat.lp['meshfn_exten'] + '.pkl'):
                print 'saving ipr ', ii, ' of ', len(self.haldane_lattices), '...'
    ...
    def ensure_all_polygons_saved(self):
        ...
        for ii in range(len(self.haldane_lattices)):
            ...
            if not glob.glob(glat.lp['meshfn'] + '/polygons.pkl'):
                print 'saving polygons ', ii, ' of ', len(self.haldane_lattices), '...'
    ...
    def ensure_all_prpoly_saved(self):
        ...
        for ii in range(len(self.haldane_lattices)):
            ...
            if not glob.glob(glat.lp['meshfn'] + '/prpoly' + glat.lp['meshfn_exten'] + '.pkl'):
                print 'saving prpoly ', ii, ' of ', len(self.haldane_lattices), '...'
    ...
    def plot_DOS_overlay(self, outdir=None, title=r'$D(\omega)$ for Haldane networks', fname='DOS_overlay.png',
        ...
        for ii in range(len(self.haldane_lattices)):
            ...
            if ii == 0:
                fig, DOS_ax = leplt.initialize_DOS_plot(eigval, 'haldane', pin=-5000, alpha=alpha)
            else:
                leplt.DOS_plot(eigval, DOS_ax, 'haldane', pin=-5000, alpha=alpha)
    ...
    def plot_ipr_DOS_stack(self, outdir=None, title=r'$D(\omega)$ for Haldane network', fname='ipr_DOS_stack',
        ...
        **kwargs: arguments passed to glat.add_ipr_to_ax()
        ...
        fig, ax = plt.subplots(len(self.haldane_lattices), 1, sharex=True, sharey=True)
        ...
        for ii in range(n_glats):
            ...
            DOSaxis = self.haldane_lattices[ii].add_ipr_to_ax(ax[ii], alpha=alpha, inverse_PR=inverse_PR,
    ...
    def plot_ipr_DOS_overlay(self, outdir=None, title=r'$D(\omega)$ for Haldane network', fname='ipr_DOS_overlay',
        ...
        **kwargs: most of lepm.plotting.plotting.colored_DOS_plot() keyword arguments
        ...
        for ii in range(len(self.haldane_lattices)):
            ...
            if self.haldane_lattices[ii].ipr is None:
                ...
                try:
                    ...
                    if self.haldane_lattices[ii].eigvect is None:
                        eigvect = self.haldane_lattices[ii].load_eigvect(attribute=False)
                    else:
                        eigvect = self.haldane_lattices[ii].eigvect
                    ipr = self.haldane_lattices[ii].calc_ipr(eigvect=eigvect, attribute=False)
            ...
            if ii == 0 and initialize and DOS_ax is None:
                if inverse_PR:
                    fig, DOS_ax, cbar_ax, cbar = leplt.initialize_colored_DOS_plot(eigval, 'haldane', pin=-5000,
                ...
                if inverse_PR:
                    leplt.colored_DOS_plot(eigval, DOS_ax, 'haldane', pin=-5000, alpha=alpha, colorV=ipr, fontsize=FSFS,
                                           colormap='viridis', linewidth=0, make_cbar=False, vmin=vmin,
                                           vmax=vmax, **kwargs)
                else:
                    leplt.colored_DOS_plot(eigval, DOS_ax, 'haldane', pin=-5000, alpha=alpha, colorV=1./ipr, fontsize=FSFS,
    ...
    def plot_ill_DOS_overlay(self, outdir=None, title=r'$D(\omega)$ for Haldane network', fname='ipr_DOS_overlay',
                             show=False, close=True, dos_ax=None, cbar_ax=None, check=False, fontsize=8, **kwargs):
        """Plot localization-colored DOS of the Haldane networks and overlay them
        ...
        **kwargs: lepm.plotting.plotting.colored_DOS_plot() keyword arguments
            Excluding fontsize
        """
        dos_ax, cbar_ax = hcpfns.plot_ill_dos_overlay(self, dos_ax=dos_ax, cbar_ax=cbar_ax, fontsize=fontsize, **kwargs)
        if check:
            plt.pause(1)
    ...
    def plot_prpoly_overlay(self, outdir=None, title=r'Polygonal contributions to normal mode excitations',
                            fname='prpoly_haldane_hist_overlay', fontsize=8, show=True,
                            shaded=False, vmax=None):
        """Plot Inverse Participation Ratio of the Haldane network for each n-polygon
        ...
        """
        n_glats = max(len(self.haldane_lattices), len(self.meshfns))
        ...
        ind = 0
        for glat in self.haldane_lattices:
            print '\nAdding the', ind, 'th haldane_lattice to prpoly plot...'
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Specify parameters for HaldaneCollection class.')
    ...
    if args.prpoly_overlay:
        ...
        python haldane_collection.py -prpoly_overlay -LT hucentroid -NP 20 -periodic
        """
