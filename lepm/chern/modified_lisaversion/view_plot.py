...
def make_plot(kkx, kky, bands, traces, tvals, ons, pin, num='all'):
    ...
    ax1 = sps.axes_in_mm(0, h - h1, w1, h1, label=None, label_params=label_params,
                         projection='3d')  # x0, y0, width, height
    ax2 = sps.axes_in_mm(0, h - 2 * h1, w1, h1, label=None, label_params=label_params,
                         projection='3d')  # x0, y0, width, height
    ...
    ax3.axis([0, 1, 0, 1.])
    ax3.text(0.025, .75, '$k_1 = %.2f$' % tvals[0], style='italic',
             color=cc1,
             fontsize=12
             )
    ax3.text(0.525, .75, '$k_2 = %.2f$' % tvals[1], style='italic',
             color=cc2,
             fontsize=12
             )
    ax3.text(0.025, .7, '$k_3 = %.2f$' % tvals[2], style='italic',
             color=cc3,
             fontsize=12
             )
    ax3.text(0.525, .7, '$k_4 = %.2f$' % tvals[3], style='italic',
             color=cc4,
             fontsize=12
             # bbox={'facecolor':'red', 'alpha':0.5, 'pad':10}
             )

    ax3.text(0.025, .65, '$M_1 = %.2f$' % ons[0], style='italic',
             color=c1,
             fontsize=12
             )
    ax3.text(0.525, .65, '$M_2 = %.2f$' % ons[1], style='italic',
             color='#B9B9B9',
             fontsize=12
             )
    ax3.text(0.025, .6, '$M_3 = %.2f$' % ons[2], style='italic',
             color=c3,
             fontsize=12
             )
    ax3.text(0.525, .6, '$M_4 = %.2f$' % ons[3], style='italic',
             color=c4,
             fontsize=12
             # bbox={'facecolor':'red', 'alpha':0.5, 'pad':10}
             )
    ...
    nb = len(bands[0])
    b = []
    bz_area = PolygonArea(vertex_points)
    band_gaps = zeros(nb - 1)
    ax3.text(0.025, .55, '$\mathrm{Chern\/ numbers}$', fontweight='bold',
             color='k',
             fontsize=12

             )

    ax3.text(0.025, .35, '$\mathrm{Band\/ boundaries}$', fontweight='bold',
             color='k',
             fontsize=12

             )

    ax3.text(0.025, .15, '$\mathrm{Min.\/differences}$', fontweight='bold',
             color='k',
             fontsize=12
    ...
    for i in range(int(nb)):
        ...
        if j >= nb / 2:
            if num == 'all':
                ...
                # ax2.plot_trisurf(kx, ky, bands[:,j], cmap = 'cool', vmin = min(abs(bands.flatten())), vmax=max(abs(bands.flatten())), linewidth = 0.0)
            elif j == num:
            ...
            bv = ((1j / (2 * pi)) * mean(traces[:, i]) * bz_area)
            if abs(bv) > 0.7:
                cc = 'r'
            else:
                cc = 'k'
        ...
        b.append((1j / (2 * pi)) * mean(traces[:, i]) * bz_area)
...
def c_3D_plot(x, y, z, vertex_points, ax, v_min, v_max):
    ...
    vp = array([array([xmid[i], ymid[i]]).T for i in range(len(xmid))])
    areas = array([PolygonArea(vp[i]) for i in range(len(vp))])
    ...
    mask = array([not poly_path.contains_point([xmid[i], ymid[i]]) for i in range(len(xmid))])
...
def movie_honeycomb_single_frame(deltaz, phi, text=True):
    ...
    """
    a1, a2 = bz.find_lattice_vecs(deltaz * pi / 180., phi * pi / 180.)
...
if __name__ == '__main__':
    ...
    tot_path = '/Users/lisa/Dropbox/Research/Chern_number_calculation/phase_sheared/save_all_try2/data_dict45.0_195.0.pickle'
    ...
    kx = array(data['kx'])
    ky = array(data['ky'])
    traces = array(data['traces'])
    bands = array(data['bands'])
    ons = array(data['ons'])
    tvals = array(data['tvals'])
    pin = 1
