...
"""Auxiliary functions for computing the Chern number in k space (integrating curvature over BZ using Monte Carlo)"""
...
def ang_fac(ang):
    """return the angular factor to go into exponential (i2theta)"""
...
def make_M_magnetic(k, angs, num_neis, bls, tvals, ons, aol=0.8):
...
def outer_hexagon(size):
    return np.array([(size * np.sin(i * np.pi / 3), size * np.cos(i * np.pi / 3)) for i in range(6)]), 'hexagon'
...
def calc_p(eigvect):
    ...
    le = len(eigvect)
    for j in range(le):
        p.append(np.conjugate(np.reshape((eigvect)[j], [le, 1])) * np.reshape(eigvect[j], [1, le]))

    return [p, eigvect]


def func(g):
    return lambda k: make_f(g, k)
...
def make_f(g, k):
    ...
    obj = [np.array(p_v), eigval, eigvect]
...
def generate_kx_ky_grid(delta_grid, vertex_points, crx=4, cry=4):
    ...
    i_vals = np.array([i for i in range(NP) if poly_path.contains_point([kx[i], ky[i]])])

    return kx[i_vals], ky[i_vals]
...
def calc_bands(mM, Kx, Ky):
    ...
    for j in range(len(p_v)):
        t1 = np.dot(fpx[j], np.dot(p_v[j], fpy[j]))
        t2 = np.dot(fpy[j], np.dot(p_v[j], fpx[j]))
...
def generate_kx_ky_random(NP, vertex_points, crx=6, cry=6, len_prev=0, **kwargs):
    """Generate random kx and dy values inside the BZ
    ...
    if 'center' in kwargs:
        ...
        kx = sl * (np.random.rand(NP_new) - 0.5) + center[0]
        ky = sl * (np.random.rand(NP_new) - 0.5) + center[1]
    else:
        kx = 2 * crx * np.random.rand(NP_new) - crx
        ky = 2 * cry * np.random.rand(NP_new) - cry

    poly_path = Path(vertex_points)
    i_vals = np.array([i for i in range(NP_new) if poly_path.contains_point([kx[i], ky[i]])])
    ...
    while len(kx) < (NP - len_prev):
        ...
        kkx = 2 * crx * np.random.rand(NP_new) - crx
        kky = 2 * cry * np.random.rand(NP_new) - cry
        i_vals = np.array([i for i in range(len(kkx)) if poly_path.contains_point([kkx[i], kky[i]])])

        kkx = kkx[i_vals]
        kky = kky[i_vals]

        kx = np.concatenate((kx, kkx))
        ky = np.concatenate((ky, kky))
