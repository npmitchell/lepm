import numpy as np
import lepm.chern.kchern_magnetic_gyro_functions as kcmgfns
...
'''
Class for k-space Chern calculations for gyro-magnet networks
'''


class KChernMagneticGyro:
    """kspace Chern calculation class
    """
    def __init__(self, mgyro_lattice, cp=None, cpmeshfn=None):
        ...
        if 'cpmeshfn' not in cp:
            if cpmeshfn is None:
                cpmeshfn = kcmgfns.get_cpmeshfn(cp, self.mgyro_lattice.lp)
    ...
    def get_chern(self, attribute=True, verbose=True, save_png=True):
        ...
        chern = kcmgfns.load_chern(self, verbose=verbose)
        if chern is None:
            chern = kcmgfns.calc_chern(self, verbose=verbose)
            kcmgfns.save_chern(self, verbose=verbose, save_png=save_png)
        if attribute:
            self.chern = chern
...
if __name__ == '__main__':
    ...
    import lepm.magnetic_gyro_lattice_scripts as mglatscripts
    # import lepm.magnetic_gyro_lattice_scripts_twistbc as mglattwistscripts
    # import lepm.magnetic_gyro_lattice_scripts_twist2d as mglattwist2dscripts
    ...
    if args.calc_chern:
        ...
        kchern = KChernMagneticGyro(glat, cp=cp)
        kchern.ensure_chern()
