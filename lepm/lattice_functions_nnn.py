...
def calc_NLNNN_and_KLNNN(xy, BL, NL, KL, PVx=None, PVy=None, polygons=None, ignore_tris=False):
    ...
    KLNNN : NP x maxNNN int array
        Connectivity array: ith row gives orientation (cc (+1) or clockwise (-1)) of connected next nearest neighbors
        of ith particle. ccw cyclic NNNs have bond strength +1, clockwise cyclic NNNs have bond strength -1.
    """
    # preallocate
...
def periodic_NNN_bond_angles(xy, NL, KL, NLNNN, KLNNN, PVx, PVy, cwccw=False):
...
def calc_pvnnn(lat, attribute=False):
            ...
            for jj in np.where(np.abs(KLNNN[ii]) > 0)[0]:
                ...
                if kljnnn[ii, jj] < -0.5:
                    ...
                    if nljnnn[ii, jj] == 0:
                        ...
                        col = np.where(NL[ii] == nljnnn[ii, jj])[0]
...
def calc_pv_intnnn(intnn, lat):
    ...
    if intnn == 1:
        ...
        # Get NLNNN, nljnnn, kljnnn, klknnn
        pvxnn = calc_pvnnn(lat, attribute=False)
        pvynn = calc_pvnnn(lat, attribute=False)
        if intnn > 2:
