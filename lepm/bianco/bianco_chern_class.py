...
from lepm.bianco import bianco_chern_functions
...
class BiancoChern:
    ...
    def __init__(self, haldane_lattice, cp=None, cpmeshfn=None):
        ...
        if 'cpmeshfn' not in cp:
            if cpmeshfn is None:
                cpmeshfn = bianco_chern_functions.get_cpmeshfn(cp, self.haldane_lattice.lp)
    ...
    def get_bianco_chern(self, proj=None, save=True, skip_paramsregs=False, check=False, attribute_evs=False,
        ...
        if self.chern_finsize is None or self.params_regs is None:
            ...
            if data_on_disk:
                ...
                h5fn = self.haldane_lattice.lp['rootdir'] + 'bianco_chern/' + self.haldane_lattice.lp['LatticeTop'] + '.hdf5'
                ...
                if data_on_hdf5:
                    ...
                    self.calc_bianco_chern(proj=proj, check=check, paramsregs=not skip_paramsregs,
    ...
    def calc_bianco_chern(self, proj=None, paramsregs=True, contributions=False,
        ...
        """
        print 'BiancoChern.calc_bianco_chern: calculating chern...'
        chern_finsize, params_regs, contribs = \
            bianco_chern_functions.calc_bianco_chern(self.haldane_lattice, self.cp, pp=proj,
    ...
    def get_projector(self, attribute_evs=False):
        ...
        proj = bianco_chern_functions.calc_projector(self.haldane_lattice, omegac, attribute=attribute_evs)
        return proj
    ...
    def save_chern(self):
        ...
        # Save chern_finsize
        fn = self.cp['cpmeshfn'] + 'chern_finsize.txt'
        header = 'Nreg, ksize_frac, ksize, ksys_size (note this is NP_summed), ksys_frac, nu' + \
        ...
        le.save_dict(self.haldane_lattice.lp, self.cp['cpmeshfn'] + 'lattice_params.txt', header)
...
if __name__ == '__main__':
    ...
    # Example usage:
    python bianco_chern_class.py -LT hexagonal -N 3 -shape hexagon -ksize_frac_array 0.0:0.1:1.2 -calc_chern
    python bianco_chern_class.py -LT hucentroid -N 20 -shape square -ksize_frac_array 0.0:0.1:1.2 -calc_chern

    # Example for contributions
    python bianco_chern_class.py -save_ims -LT hex_kagcframe -shape circle -alph 1.0 -N 30 -contributions -modsave 1
    '''
    ...
    if args.calc_chern:
        ...
        hlat.load()
        print '\n\n\n', hlat.lattice.lp, '\n'
        chern = BiancoChern(hlat, cp=cp)
        if args.title == '':
            title = None
        else:
            print 'title = ', eval("r'" + args.title.replace('_', ' ') + "'")
            title = eval("r'" + args.title.replace('_', ' ') + "'")
            title += r', with $t_1 = $' + le.float2pstr(lp['t1'])
            title += r' $t_2 = $' + le.float2pstr(lp['t2'])
            if hlat.lp['V0_pin_gauss']:
                title += r' $V \pm \sigma = $' + le.float2pstr(lp['pin']) + r'$\pm$' + le.float2pstr(lp['V0_pin_gauss'])
            else:
                title += r' $V = $' + str(int(lp['pin']))
        chern.calc_bianco_chern(check=args.check, title=title)
        chern.save_chern()
    ...
    if args.characterize_projector:
        ...
        print 'proj = ', proj
        dists, magproj, evxymag = bianco_chern_functions.characterize_projector(proj, chern.gyro_lattice)
    ...
    if args.characterize_projector_singlept:
        ...
        '''
        Example usage:
        python bianco_chern_class.py -projector_single -proj_XY 0.0/0.0 -LT hex_kagframe -NH 11 -NV 12 -alph 0.5
        python bianco_chern_class.py -projector_single -proj_XY 0.0/0.0 -LT hex_kagcframe -N 20 -alph 1.0 -shape circle
        '''
        meshfn = le.find_meshfn(lp)
        lp['meshfn'] = meshfn
        lat = lattice_class.Lattice(lp)
        ...
        hlat = haldane_lattice_class.HaldaneLattice(lat, lp)
        hlat.load()
        print '\n\n\nbianco_chern_class: ', hlat.lattice.lp, '\n'
        chern = BiancoChern(hlat, cp=cp)
        proj = chern.get_projector()
        dists, magproj, evxymag = bianco_chern_functions.characterize_projector(proj, chern.haldane_lattice, check=True)
        ...
        if args.proj_ind < 0:
            xyloc = sf.string_sequence_to_numpy_array(args.proj_XY, dtype=float)
            print 'xyloc = ', xyloc
