...
from lepm.bott.bott_class import BottIndex
import socket
import os
import copy
...
check = True
# check = False
addstr = '_hucentroid' #_logproduct_of_evs'
...
if lp0['LatticeTop'] != 'hexagonal':
    testvals = np.arange(0, 0.5, 0.1)
    testvals = np.hstack((testvals, np.arange(0.5, 0.8, 0.01)))
    testvals = np.hstack((testvals, np.arange(0.8, 2.0, 0.2)))
else:
    testvals = np.arange(0, 0.5, 0.1)
    testvals = np.hstack((testvals, np.arange(0.5, 0.53, 0.001)))
    testvals = np.hstack((testvals, np.arange(0.53, 0.6, 0.02)))
    testvals = np.hstack((testvals, np.arange(0.6, 2.0, 0.2)))
...
for ntest in testvals:
    ...
    pp = hcfns.calc_projector(hlat, omegac, attribute=True)
    ...
    if alternative:
        ...
        usub = eigvect[eigval > omegac]

        # U1 = la.inv(U)
        #
        # MM = np.zeros((len(U), len(U)), dtype=float)
        # le.plot_complex_matrix(U1 - U.T, show=True, name=r'$U^{-1} - U^{T}$')
        #
        # for ii in range(len(eigval)):
        #     if eigval[ii] > omegac:
        #         MM[ii, ii] = 1.0
        ...
        uu = np.dot(usub, np.dot(np.exp(1j * theta) * np.identity(len(xx)), usub.conj().transpose()))
        vv = np.dot(usub, np.dot(np.exp(1j * phi) * np.identity(len(xx)), usub.conj().transpose()))
...
# plt.tick_params(axis='both', labelsize=tickfs)
if alternative:
    name = '/Users/npmitchell/Desktop/temp/shorthand' + addstr
else:
    name = '/Users/npmitchell/Desktop/temp/dirty' + addstr

plt.savefig(name + '_vpin0p10_abtransition_n' + str(lp['NH']) + '.png')
if lp['LatticeTop'] == 'hexagonal':
    ax.set_xlim(0.49, 0.53)
    ax.xaxis.set_ticks([0.5, 0.51, 0.52])
else:
    ax.set_xlim(0.58, 0.7)
    ax.xaxis.set_ticks([0.6, 0.65, 0.7])
