...
def data_on_disk(bott):
    ...
    if glob.glob(bott.cp['h5fn']):
        ...
        print 'bott_magnetic_gyro_functions: looking in ', bott.cp['h5fn'], '\n for ', bott.cp['h5subgroup']
...
def calc_small_projector(magnetic_gyro_lattice, omegac, attribute=False, save_eigv=False, force_hdf5_eigv=True):
    ...
    Parameters
    ----------
    magnetic_gyro_lattice : HaldaneLattice instance
    ...
    eigval, eigvect = magnetic_gyro_lattice.get_eigval_eigvect(attribute=attribute)
    if save_eigv:
        magnetic_gyro_lattice.save_eigval_eigvect(eigval=eigval, eigvect=eigvect, force_hdf5_eigv=force_hdf5_eigv)
...
def calc_bott(gyro_lattice, cp, psub=None, check=False, verbose=False, attribute_evs=False,
    ...
    if psub is None:
        ...
        psub = calc_small_projector(gyro_lattice, omegac, attribute=attribute_evs, save_eigv=save_eigv,
...
def get_cmeshfn(lp, rootdir=None):
    ...
    cmeshfn += 'bott_magnetic_gyro/'
    for strseg in meshfn_split[(ind + 1):]:
        cmeshfn += strseg + '/'

    # Form physics subdir for shortrange, aoverl, Omk, Omg, V0_pin_gauss, V0_spring_gauss
    cmeshfn += 'aol' + sf.float2pstr(lp['aoverl'], ndigits=8) + '/'
    if 'shortrange' in lp['meshfn_exten']:
        cmeshfn += 'shortrange/'
...
if __name__ == '__main__':
    ...
    # physics
    parser.add_argument('-shortrange', '--shortrange', help='Consider magnetic couplings only to nearest neighbors',
                        action='store_true')
