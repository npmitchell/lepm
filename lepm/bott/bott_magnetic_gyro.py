...
import lepm.bott.bott_magnetic_gyro_functions as bmgfns
...
class MagneticGyroBott:
    ...
    mgyro_lattice : a magnetic_gyro_lattice instance
    ...
    """
    def __init__(self, mgyro_lattice, cp=None, cpmeshfn=None, h5fn=None):
        self.mgyro_lattice = mgyro_lattice
        self.bott = None
        self.params_regs = {}
        ...
        if 'cpmeshfn' not in cp:
            if cpmeshfn is None:
                cpmeshfn = bmgfns.get_cpmeshfn(cp, self.mgyro_lattice.lp)
            cp['cpmeshfn'] = cpmeshfn

        # Get subdir path from h5fn
        hfnsplit = cp['cpmeshfn'].split('/' + self.mgyro_lattice.lp['LatticeTop'] + '/')
    ...
    def get_bott(self, proj=None, save=True, check=False, attribute_evs=False, verbose=False,
        ...
        """
        Load or compute bott index on self.mgyro_lattice with bott parameters self.cp
        ...
        if self.bott is None:
            ...
            # if verbose:
            print 'MagneticGyroBott: Searching for saved bott: ', self.cp['cpmeshfn']
            ...
            data_on_disk = bmgfns.data_on_disk(self)
            if not data_on_disk:
                print 'searching for bott as text file...'
                data_on_disk = bmgfns.data_on_disk_txt(self)

            if data_on_disk:
                print '\nMagneticGyroBott: Found bott on hdf5 or txt file, loading...'
                self.load_bott()
            else:
                # Otherwise calculate it
                print 'MagneticGyroBott: data_on_disk = ', data_on_disk
                # print 'bott_gyro: could not find bott saved, exiting...'
                # sys.exit()
                print '\n'
                print 'MagneticGyroBott : calculating bott...'
                self.calc_bott(proj=proj, check=check, attribute_evs=attribute_evs, verbose=verbose,
                               save_eigv=save_eigv, force_hdf5_eigv=force_hdf5_eigv)

                if save:
                    print 'MagneticGyroBott: saving bott...'
                    if save_as_txt:
                        self.save_bott_txt()
                    else:
                        self.save_bott()
    ...
    def calc_bott(self, proj=None, check=False, verbose=False, attribute_evs=False,
                  save_eigv=False, force_hdf5_eigv=True):
        """
        Compute Bott index on self.mgyro_lattice with calculation parameters self.cp
        """
        self.bott = bmgfns.calc_bott(self.mgyro_lattice, self.cp, psub=proj, check=check,
                                     verbose=verbose, attribute_evs=attribute_evs,
                                     save_eigv=save_eigv, force_hdf5_eigv=force_hdf5_eigv)
        return self.bott

    def get_projector(self, attribute_evs=False, save_eigv=False, force_hdf5_eigv=True):
        """Obtain projection operator for current instance of BottIndex class

        Parameters
        ----------
        attribute_evs : bool
            Attribute eigvect/eigval to self.mgyro_lattice during computation of projector
        ...
        proj = bmgfns.calc_small_projector(self.mgyro_lattice, omegac, attribute=attribute_evs,
                                           save_eigv=save_eigv, force_hdf5_eigv=force_hdf5_eigv)
        return proj
    ...
    def save_bott(self):
        ...
        for key in self.mgyro_lattice.lp:
            lpgroup.attrs[key] = self.mgyro_lattice.lp[key]
    ...
    def save_bott_txt(self):
        ...
        header = 'Lattice parameters, copied from meshfn: ' + self.mgyro_lattice.lp['meshfn']
        dio.save_dict(self.mgyro_lattice.lp, self.cp['cpmeshfn'] + 'lattice_params.txt', header)
...
if __name__ == '__main__':
    ...
    python ./bott/bott_magnetic_gyro.py -LT hexagonal -N 15 -save_as_txt -delta 1.275 -hexbow_omegac -calc_bott

    python bott_magnetic_gyro.py -LT hucentroid -N 30 -shape square -contributions -ksize_frac_array 0.0:0.1:1.2
    python bott_magnetic_gyro.py -save_ims -LT hex_kagcframe -shape circle -alph 0.3 -calc_bott -N 30 -modsave 1
    python ./kitaev/bott_magnetic_gyro.py -LT accordionkag -N 5 -shape square -contributions -Nks 110 -intparam 2 -alph 1.0
    python ./kitaev/bott_magnetic_gyro.py -LT accordionkag -N 6 -shape square -calc_bott -Nks 110 -intparam 2 -alph 0.85

    # Example for contributions
    python bott_magnetic_gyro.py -save_ims -LT hex_kagcframe -shape circle -alph 1.0 -N 30 -contributions -modsave 1
    '''
    ...
    if args.calc_bott:
        """Example usage:
        python ./bott/bott_magnetic_gyro.py -LT hexagonal -shape square -N 5 -calc_bott
        """
        ...
        mglat.load()
        print '\n\n\n', mglat.lattice.lp, '\n'
        mbott = MagneticGyroBott(mglat, cp=cp)

        mbott.calc_bott(check=args.check, verbose=args.verbose)

        # Ensure the gyro lattice
        # xy = mbott.mgyro_lattice.lattice.xy
        # plt.plot(xy[:, 0], xy[:, 1], 'b.')
        # plt.show()
        # sys.exit()

        mbott.save_bott_txt()
        print 'mbott = ', mbott.bott
    ...
    if args.ensure_bott:
        ...
        mglat.load()
        print '\n\n\n', mglat.lattice.lp, '\n'
        bott = MagneticGyroBott(mglat, cp=cp)
