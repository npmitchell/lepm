...
import lepm.bott.bott_class as bott_class
...
import lepm.plotting.bott_collection_plotting_functions as kcollpfns
...
class BottGyroCollection:
    ...
    where bott is a class with attributes cp, chern_finsize array.
        cp : dict
            keys : str
    ...
    self.botts : dict
        keys are glat_names (strings), values are lists of BottIndex instances
    ...
    def bott_is_saved(self, glat, cp=None, verbose=False):
        if verbose:
            print '\nChecking if bott is already saved...'
        if cp is None:
            cp = self.cp
        bott = bott_class.BottIndex(glat, cp)
        if glob.glob(bott.cp['cpmeshfn'] + 'bott.txt'):
            return True
        else:
            return False

    def add_bott(self, glat, cp=None, proj=None, attribute_evs=False, verbose=False):
        """Add a BottIndex instance to the BottGyroCollection, and append the glat_name to self.glat_names if not
        ...
        for glat in glatstodo:
            ...
            for val in paramtodo:
                ...
                if varyloc:
                    ...
                    # Add one bott instance for each location, given that the projector is computed
                    ...
                    for xx in xxVtodo:
                        for yy in yyVtodo:
                            ...
                            if not self.bott_is_saved(glat, cp=cp) and proj is None:
                                print '\nkcoll: bott is not saved, computing projector...'
                                print '... for cpmeshfn = ', cp['cpmeshfn']
                                proj = bgfns.calc_projector(glat, self.cp['omegac'][0])
    ...
    def plot_botts_omegac(self, ngrid=None, filename='omegac_overlay', vmin=-1.0, vmax=1.0,
        ...
        for glat_name in self.botts:
            ...
            # Save the plot
            glat_cmesh = bgfns.get_cmeshfn(self.botts[glat_name][ind].gyro_lattice.lp,
                                                     rootdir=self.botts[glat_name][ind].cp['rootdir'])
            print 'saving plot to ' + glat_cmesh + filename
            bpfns.plot_spectrum_with_dos(glat, ksys_fracM, omegacM, nuM, ngrid,
    ...
    def plot_bott_spectrum_on_axis(self, ax, dos_ax, cbar_ipr_ax, cbar_nu_ax, **kwargs):
        return kcollpfns.plot_bott_spectrum_on_axis(self, ax, dos_ax, cbar_ipr_ax, cbar_nu_ax, **kwargs)
        ...
        for glat_name in self.botts:
            ...
            for bott in self.botts[glat_name]:
                if first:
                    ...
                    if (bott.chern_finsize[:, -2] == ksys_size).all():
                        params.append(bott.gyro_lattice.lp[param])
                        nu_tmp = bott.chern_finsize[:, -1]
                        ...
                        bad = np.where(np.abs(bott.chern_finsize[:, -2] - ksys_size) > 1e-3)[0]
                        print 'kcoll: bott.chern_finsize[bad, :] = ', bott.chern_finsize[bad, :]
                        print 'kcoll: ksys_size[bad] = ', ksys_size[bad]
                        plt.plot(np.arange(len(ksys_size)), ksys_size, 'bo-')
                        plt.plot(np.arange(len(ksys_size)), bott.chern_finsize[:, -2], 'r.-')
                        plt.show()
    ...
    def plot_bottspectra_vary_glatparam(self, param_om_nu=None, param='percolation_density', ngrid=None,
